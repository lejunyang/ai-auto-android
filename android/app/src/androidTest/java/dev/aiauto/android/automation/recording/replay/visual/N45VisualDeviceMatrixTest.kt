...
class N45VisualDeviceMatrixTest {
    ...
    fun runVisualActionMatrixCase() {
        ...
        try {
            ...
            SecureAccessibilitySettings(
                ...
                verifyMatrix(testActivity, harness, instrumentation, identity)
            }
        } finally {
            activity?.finish()
            instrumentation.waitForIdleSync()
    ...
    private fun verifyMatrix(
        testActivity: ScreenshotTestActivity,
        harness: N45VisualDeviceHarness,
        ...
        assertThrows(IllegalStateException::class.java) {
            harness.assertIdentityUnchanged(identity.copyWith(serial = "emulator-5556"))
        }
        assertThrows(IllegalStateException::class.java) {
            harness.assertIdentityUnchanged(identity.copyWith(serial = "physical-device"))
        }
        assertThrows(IllegalStateException::class.java) {
            harness.assertIdentityUnchanged(identity.copyWith(fingerprint = "f".repeat(64)))
        }
        assertThrows(IllegalStateException::class.java) {
            harness.assertIdentityUnchanged(identity.copyWith(marker = "WRONG_TEST_MARKER"))
        }
    }
