...
// 脚本用途：在 N31 固定 profile 上逐轮恢复 clean snapshot 并运行 N33 原生完整设备场景。
...
const testClass = "dev.aiauto.fixture.NativeFixtureDeviceTest";
const testMethod = "runSelectedFixtureScenario";
...
const assertEmulatorState = async (command, aactlPath, expectedSerial) => {
...
export const runNativeFixtureMatrix = async ({
  argv,
  environment,
  dependencies = {},
}) => {
  ...
  const createTemporaryRoot = dependencies.createTemporaryRoot
    ?? (() => mkdtemp(path.join(roots.stateRoot, "n33-matrix-")));
  ...
  const persistReport = dependencies.persistReport ?? (async (report) => {
    ...
    await writeFile(
      path.join(reportDirectory, `n33-native-matrix-${profile.id}.json`),
  ...
  try {
    ...
    assertSuccess(
      await command(gradlew, [
        ...
        ":device-fixture:assembleDebugAndroidTest",
      ], {
        cwd: repositoryRoot,
        env: environment,
    ...
    await assertEmulatorState(command, aactlPath, null);
    ...
    await assertEmulatorState(command, aactlPath, booted.serial);
    const rounds = [];
    for (let round = 1; round <= roundsPerProfile; round += 1) {
      ...
      const result = await command(gradlew, [
        ...
        "android",
        "--no-daemon",
        ":device-fixture:connectedDebugAndroidTest",
        `-Pandroid.testInstrumentationRunnerArguments.class=${testClass}`,
        "-Pandroid.testInstrumentationRunnerArguments.fixtureScenario=all",
        "-Pandroid.testInstrumentationRunnerArguments.fixtureRepeat=1",
      ], {
        cwd: repositoryRoot,
      ...
      if (junit.status === "infrastructure-failure") {
        fail("MATRIX_INFRASTRUCTURE_FAILURE");
      }
    }
