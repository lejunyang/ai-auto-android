...
ADB 使用 USB 调试接口，不依赖 MTP“传输文件”或 PTP“传输图片”协议。手机的
USB 用途通常可以保持“仅充电”，前提是线缆支持数据且 USB 调试链路正常。若设备
始终不出现，可临时切换到“传输文件”触发部分厂商设备重新枚举或加载 Windows
驱动；无需为了 ADB 选择“传输图片”。

首次接入和完整功能验收见[真机 USB 验收](device-usb-acceptance.md)。

### macOS

Android 官方说明 macOS 不需要额外 USB 驱动。设备未出现时依次检查：

1. 使用确认支持数据传输的线缆，不使用物理上只有供电线芯的充电专用线；
2. 设备保持解锁，开启“开发者选项 > USB 调试”；
3. 更换电脑 USB 端口、线缆，避免未经供电的 Hub；
4. USB 用途保持“仅充电”并重试；若厂商实现仍未枚举调试接口，再临时切换
   “传输文件”，不需要选择“传输图片”；
...
- 只有便携设备、没有 ADB Interface：确认 USB 调试和驱动；可临时切换
  “传输文件”触发驱动重新绑定，但 ADB 本身不依赖文件传输模式；
...
- `SCRIPT_NOT_FOUND`：先确认 Bridge 会话有效，再列出当前 App 私有存储中的脱敏
  脚本摘要并重新选择 UUID：
  ...
  列表只返回 `id`、名称、目标包、步骤数、创建时间和 requirements，不返回步骤、
  变量或 secret。
- `SECRET_REQUIRED`：含 secret 的脚本必须从 App 详情页提供临时值并回放；
  CLI/MCP 当前不能传 secret。
- 只录到窗口变化：当前发布配置已订阅点击、长按、文本、滚动、窗口状态和窗口集合
  变化。确认 App 录制状态为“正在录制”、录制页填写的目标包与当前前台包完全一致，
  且目标控件确实发出 Accessibility 事件。WebView、自绘 Canvas、游戏和物理系统
  导航可能没有稳定的语义事件；不要改用原始触摸或猜测坐标伪造步骤，详见
  [录制与回放](recording.md)。
